...
describe('sanitizeTrackPoints', () => {
  it('removes duplicate timestamps', () => {
    const result = sanitizeTrackPoints([
      ...
      { lat: 56.952, lng: 24.102, timestamp: '2026-06-21T10:00:10.000Z' },
    ]);

    expect(result).toHaveLength(2);
    expect(result[1].lat).toBe(56.952);
  });
  ...
  it('drops GPS spike points with impossible speed', () => {
    const result = sanitizeTrackPoints([
      ...
      { lat: 56.961, lng: 24.111, timestamp: '2026-06-21T10:00:15.000Z' },
    ]);

    expect(result).toHaveLength(2);
    expect(result[1].lat).toBe(56.961);
  });
